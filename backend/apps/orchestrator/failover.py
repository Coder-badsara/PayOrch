...
from apps.payments.models import GatewayName, TransactionStatus, GatewayRoute
...
class FailoverService:
    ...
    def execute_with_failover(
        ...
        if preferred_gateway:
            ...
            try:
                ...
                if getattr(settings, 'DEBUG', False) and getattr(settings, 'FORCE_GATEWAY_SUCCESS', False):
                    ...
                    self._record_attempt(transaction_id, preferred_gateway, order_params, result, latency=0)
                ...
                self._record_attempt(transaction_id, preferred_gateway, order_params, result, latency=latency)
                routing_history.append({"gateway": preferred_gateway.value, "status": "SUCCESS"})
                return preferred_gateway, result, routing_history
                ...
                self._record_attempt(
                    transaction_id, preferred_gateway, order_params,
        ...
        for attempt_idx in range(max_attempts):
            try:
                # 1. Select gateway
                # The router will use weights as per PDF A3.1
                ...
                try:
                    if getattr(settings, 'DEBUG', False) and getattr(settings, 'FORCE_GATEWAY_SUCCESS', False):
                        ...
                        self._record_attempt(transaction_id, gateway_name, order_params, result, latency=0)
                    ...
                    latency = int((time.time() - start_time) * 1000)
                    self._record_attempt(transaction_id, gateway_name, order_params, result, latency=latency)
                    routing_history.append({"gateway": gateway_name.value, "status": "SUCCESS"})
                    return gateway_name, result, routing_history
                    ...
                    self._record_attempt(
                        transaction_id, gateway_name, order_params, 
                        error_code="GATEWAY_ERROR", error_message=str(e),
    ...
    def _record_attempt(
        self, transaction_id, gateway_name, order_params, 
        result=None, error_code=None, error_message=None, 
        latency=None, is_failover=False
    ):
        # Map success to AUTH_INITIATED since we are in the initiation phase
        # Failing during initiation means state remains ROUTE_SELECTED or moves to FAILED
        state = TransactionStatus.AUTH_INITIATED if result else TransactionStatus.AUTH_FAILED
        
        GatewayRoute.objects.create(
