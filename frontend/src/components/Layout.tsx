...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const isFullscreenPage = location.pathname.startsWith('/payment-success') || location.pathname.startsWith('/payment-failed');

  return (
    <div className="flex h-screen bg-gray-900 text-white">
      {/* Sidebar - Hidden on success/failed pages */}
      {!isFullscreenPage && (
        <div className="w-64 bg-gray-950 border-r border-gray-800 p-4 flex flex-col">
          <div className="items-center space-x-3 mb-10 px-2 flex">
      ...
      {/* Main Content */}
      <div className={`flex-1 overflow-auto bg-gray-900 ${isFullscreenPage ? 'p-0' : 'p-8'}`}>
        {children}
      </div>
    </div>
  );
